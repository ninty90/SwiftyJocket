...
class WebSocket : NSObject, NSStreamDelegate {
    ...
    */
     ...
     */
    func writeString(str: String, completion: (() -> ())? = nil) {
    ...
    /**
     ...
     - parameter completion: The (optional) completion handler.
     */
    func writeData(data: NSData, completion: (() -> ())? = nil) {
        guard isConnected else { return }
    ...
    func writePing(data: NSData, completion: (() -> ())? = nil) {
        guard isConnected else { return }
...
class SSLSecurity {
    ...
    var pubKeys: [SecKeyRef]? //the keys
    ...
    /**
     ...
     - parameter keys: is the certificates or keys to use
     - parameter usePublicKeys: is to specific if the publicKeys or certificates should be used for SSL pinning validation
